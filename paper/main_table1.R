...
df_elderly = readRDS("samples/coresorphanhood_any_age_group_sex_uf_allcause.RDS") 
df_both = readRDS(paste0("samples/parentalandcoresorphanhood_allcause.RDS")) %>%
...
table_one = data.frame(Region=c(sort(uf_lbls$uf_name), "Total"))
table_one = table_one %>% left_join(df_parental_table, by="Region")
table_one = table_one %>% left_join(df_elderly_table, by="Region")
table_one = table_one %>% left_join(df_both_table, by="Region")

write_clip(table_one)
